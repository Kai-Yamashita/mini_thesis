...
class VectorEnv:
    ...
    CUBE_COLOR = (160/255,64/255,0, 1)  # Yellow
    DEBUG_LINE_COLORS = [
    ...
    def __init__(
        ...
        if not any('rescue_robot' in g for g in self.robot_config):
            self.receptacle_position = (self.room_length / 3, self.room_width / 3, 0)
            self.receptacle_position2 = (0, 0, 0)
    ...
    def cube_position_in_receptacle(self, cube_position):
        ...
        #     return True
        # return False
        if (abs(cube_position[0] - self.receptacle_position[0]) <= half_width and abs(cube_position[1] - self.receptacle_position[1]) <= half_width):
            #or (abs(cube_position[0] - self.receptacle_position2[0]) <= half_width and abs(cube_position[1] - self.receptacle_position2[1]) <= half_width)
            return True
        else:
            return False
    ...
    def _create_env(self):
        ...
        # Create floor
        floor_color = (204/255, 204/255, 255/255, 1)
        floor_thickness = 10
        wall_thickness = 1.4
        ...
        floor_visual_shape_id = self.p.createVisualShape(pybullet.GEOM_BOX, halfExtents=floor_half_extents, rgbaColor = floor_color)
        ...
        obstacle_color = (143/255,188/255,143/255, 1)
        ...
        if not any('rescue_robot' in g for g in self.robot_config):
            receptacle_color = (138.0/255, 43.0 / 255, 226.0 / 255, 1)  # blueviolet
            ...
            #self.receptacle_id2 = self.p.createMultiBody(0, receptacle_collision_shape_id, receptacle_visual_shape_id, self.receptacle_position2)
...
class Robot(ABC):
    ...
    COLOR = (240.0/255, 184.0 / 255, 35.0 / 255, 1)  # Yellow
